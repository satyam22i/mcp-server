...
class WooCommerceServer {
  ...
  private initializeServices(): void {
    ...
    this.fileManager = new FileManager(
      process.env.WORDPRESS_ROOT_PATH || './wordpress',
  ...
  private setupRoutes(): void {
    const requireAuth: express.RequestHandler = (req, res, next) => {
      ...
      (req as any).wpService = session.service;
      next();
    };
    ...
    this.app.get('/oauth/callback', async (req, res) => {
      try {
        ...
        if (!tokenRes.ok) throw new Error(tokenJson.error_description || 'OAuth token exchange failed');
    ...
    this.app.post('/execute/:toolName', requireAuth, async (req, res) => {
      ...
      try {
        ...
        const wpService: WordPressService = (req as any).wpService || this.wordpressService;
