...
import { FileOperation, FileChange } from '../types/index.js';

export class FileManager {
  private wordpressRoot: string;
  private backupDir: string;
  private watcher: chokidar.FSWatcher | null = null;
  private changeCallbacks: ((change: FileChange) => void)[] = [];

  constructor(wordpressRoot: string, backupDir: string = './backups') {
  ...
  async editFile(filePath: string, edits: { line?: number; content: string; type: 'replace' | 'insert' | 'append' }[]): Promise<void> {
    try {
  ...
  async searchInFiles(searchTerm: string, filePattern: string = '*.php'): Promise<Array<{ file: string; line: number; content: string }>> {
    try {
  ...
  startFileWatching(patterns: string[] = ['**/*.php', '**/*.js', '**/*.css']): void {
    if (this.watcher) {
      this.watcher.close();
    }
  ...
  private async handleFileChange(filePath: string, type: 'created' | 'modified' | 'deleted'): Promise<void> {
